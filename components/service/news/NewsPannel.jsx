...
export default function NewsPannel({ data }) {
  ...
  return (
    <div className='shadow-card p-8 pb-2 mb-4 rounded-lg hover:translate-x-2'>
      <section className='text-lg font-medium mb-3 hover:text-blue-700 cursor-pointer'
        onClick={() => handleJump(data.link)}>
        {data.title}
      </section>
      <section className='flex gap-4'>
        {data.img ? <div className='flex-shrink-0'>
        ...
        <div className='py-4 pb-8 flex flex-col justify-between w-full'>
          ...
          {data.tag ? <section className='inline bg-orange-400 w-fit text-white p-1 rounded text-xs mt-3'>
            {data.tag}
          </section> : null}
        </div>
      </section>
